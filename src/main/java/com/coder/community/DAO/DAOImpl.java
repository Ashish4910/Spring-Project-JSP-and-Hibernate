...
//	}
	...
	@Transactional
	public List<User> getCustomerWithImages() {
		Session currSession = sessionFactory.getCurrentSession();
		Query<User> theQuery = currSession.createQuery("from User", User.class);
		List<User> theUsers = theQuery.getResultList();

		for (User user : theUsers) {
			// Assuming you have a method to retrieve image data from the database for each
			// user
			byte[] imageData = retrieveImageDataForUser(user.getId()); // Replace with your actual logic

			System.out.println("Image get Customer with Images" + imageData);
...
//			user.setImage(imageData);
	...
	@Transactional
	public byte[] retrieveImageDataForUser(int userId) {
		Session currSession = sessionFactory.getCurrentSession();

		// Replace "User" with your actual entity class name and "imageData" with the
		// actual column name
		Query query = currSession.createQuery("SELECT u.image FROM User u WHERE u.id = :userId");
		query.setParameter("userId", userId);

		byte[] imageData = (byte[]) query.uniqueResult();

		System.out.println("retrieveImageDataForUser" + imageData);
...
//	public List<User> updateUser(int theId) {
		Session currentSession = sessionFactory.getCurrentSession();
//
//		Query theQuery = currentSession.createQuery("from User where id=:theId", User.class);
...
//		for (User user : theUsers) {
//
//			byte[] imageData = retrieveImageDataForUser(user.getId());
//
//			String base64Image = Base64.getEncoder().encodeToString(imageData);
//			user.setImageBase64(base64Image);
//
//		}
		...
		theQuery.setParameter("theId", theId);
		System.out.println("User id" + theId);
		List<User> theUser = theQuery.getResultList();

		System.out.println("user details for the given id " + theUser);
		return theUser;

		// List<User> theUser = currentSession.get(User.class, theId);
		// return theUsers;
	...
	@Transactional
	public User updateUsercurret(int theId) {
		...
		System.out.println("updateUsercurret edit form initilized  " + theUser.getImageBase64());
		return theUser;
